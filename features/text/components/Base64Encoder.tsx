...
import {
  convertBase64,
  type Base64Mode,
} from '@/features/text/utils/base64Encoder'
...
const TOOL_FAQS: FAQItem[] = [
  {
    ...
    answer:
      "Base64 encoding converts binary data or text into a safe, ASCII-only format that can be transmitted over text-based protocols like email, JSON, or URLs without corruption. It's commonly used for embedding images in HTML/CSS, encoding API credentials, and transferring data.",
  },
  {
    question: 'Is Base64 encryption?',
    answer:
      'No! Base64 is NOT encryption — it is an encoding format that can be reversed by anyone without a key. It provides no security or confidentiality. If you need to protect sensitive data, use proper encryption like AES.',
  },
  {
...
const HOW_TO_STEPS = [
  {
    step: 1,
    title: 'Choose a mode',
    desc: 'Select Encode to convert text to Base64, or Decode to convert Base64 back to text.',
  },
  {
    step: 2,
...
export function Base64Encoder() {
  ...
  const result = useMemo(() => convertBase64(text, mode), [text, mode])
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[140px] rounded-xl border border-border bg-muted/30 p-4 text-foreground placeholder:text-muted-foreground resize-y font-mono text-sm transition-all"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-border bg-card p-3 text-center">
              <div className="text-lg font-bold tabular-nums">{inputChars}</div>
              <div className="text-xs text-muted-foreground">Input Chars</div>
            </div>
            <div className="rounded-xl border border-border bg-card p-3 text-center">
              <div className="text-lg font-bold tabular-nums">{outputChars}</div>
              <div className="text-xs text-muted-foreground">Output Chars</div>
            </div>
            <div className="rounded-xl border border-border bg-card p-3 text-center">
              <div className={`text-lg font-bold tabular-nums ${
                Number(sizeChange) > 0 ? 'text-amber-500' : Number(sizeChange) < 0 ? 'text-green-500' : ''
              }`}>
          ...
          {/* ── How To Use ───────────────────────────────────────────── */}
          <div>
            <h2 className="text-xl font-bold mb-6">How to Use Base64 Encoder</h2>
            ...
            <div className="border border-border rounded-xl p-6 bg-card">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold tabular-nums">
                    {result.error ? '—' : `${outputChars} chars`}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  >
                    {!hasText ? '—' : result.error ? 'Error' : 'Ready'}
                  </span>
                </div>
                {fileName && (
            ...
            <div className="border border-border rounded-xl p-6 bg-muted/20">
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li className="flex gap-2">
                  ...
                  <span>Base64 encoding increases text size by ~33%.</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-primary">•</span>
                  <span>Use <strong>Swap</strong> to quickly decode what you just encoded.</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-primary">•</span>
                  <span>Paste any valid Base64 string and switch to Decode mode.</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-primary">•</span>
