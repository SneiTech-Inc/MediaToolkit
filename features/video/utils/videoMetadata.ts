...
import { getFFmpeg } from '@/features/audio/utils/ffmpegClient'
 ...
 *
 * How it works:
 * 1. Writes the file into ffmpeg's virtual filesystem
 * 2. Runs ffmpeg with null output muxer (`-f null -`) which reads headers
 *    and prints stream info to stderr without encoding
 * 3. Parses codec, FPS, and bitrate from ffmpeg's log output
 * 4. Falls back to sensible defaults for any unparsed fields
 *
...
export async function getAdvancedMetadata(
  ...
  // Get basic metadata first for duration/width/height
  const basic = await getBasicMetadata(file)
  ...
  try {
    ...
    // We use ffmpeg's log event to capture stream info
  ...
  if (basic && basic.width > 0 && basic.height > 0) {
